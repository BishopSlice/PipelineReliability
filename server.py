...
# D-51: In-memory response cache for read-only (GET) Fivetran API calls.
# Keyed by (tool_name, args_hash). TTL of 30s — within a single run, Fivetran
# state is effectively stable; deduplicates repeated identical calls across agents.
...
def _cache_key(name: str, arguments: dict) -> str:
    payload = json.dumps({name: arguments}, sort_keys=True)
    return hashlib.md5(payload.encode()).hexdigest()


def _evict_expired() -> None:
    now = time.monotonic()
    expired = [k for k, (ts, _) in _RESPONSE_CACHE.items() if now - ts > _CACHE_TTL]
    for k in expired:
...
PARAM_DEFINITIONS = {
    ...
    "request_body": {
        ...
        "description": "Request body — either a JSON string or a JSON object. Refer to the schema file for the expected structure.",
    },
}
...
async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
    ...
    try:
        ...
        # 409 means the resource is already in the desired state (sync running, job running, etc.)
        # Signal this explicitly so agents don't treat it as a failure requiring retry.
        already_in_progress = (code == 409)
        return [TextContent(type="text", text=json.dumps({
            "error": not already_in_progress,  # 409 is not an error — it's a state signal
            "already_in_progress": already_in_progress,
            "code": code,
            "message": (
                message + " — this means the operation is already running/in progress. "
                "Treat this as confirmation the action is underway. Do NOT retry. "
                "Verify current status with the appropriate read tool."
            ) if already_in_progress else message,
            "tool": name,
        }))]
    except ValueError as e:
        # Tool not found or schema mismatch — fatal: agent must not retry this call
        msg = str(e)
        is_tool_not_found = name not in TOOLS
        return [TextContent(type="text", text=json.dumps({
            "error": True,
            "fatal": is_tool_not_found,
            "code": "TOOL_NOT_FOUND" if is_tool_not_found else "INVALID_REQUEST",
            "message": (
                f"{msg}. This tool does not exist in the Fivetran API. "
                "Do NOT retry this call. Output your finding with status=escalated immediately."
            ) if is_tool_not_found else msg,
            "tool": name,
        }))]
...
async def execute_tool(name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    """Execute the actual API call after validation. Caches GET responses for _CACHE_TTL seconds."""
    tool_config = TOOLS[name]
    method = tool_config["method"]
    endpoint_template = tool_config["endpoint"]
    cacheable = (method == "GET")  # D-51: only cache read-only calls
    ...
    # Parse request body if present — accept either a JSON string or a dict
    json_body = None
    if "request_body" in arguments:
        rb = arguments["request_body"]
        if isinstance(rb, dict):
            json_body = rb
        else:
            try:
                json_body = json.loads(rb)
            except json.JSONDecodeError as e:
                raise ValueError(f"Invalid JSON in request_body: {e}")

    # Execute request
    if tool_config.get("auto_paginate"):
        result = await fivetran_request_all_pages(endpoint)
    else:
        result = await fivetran_request(method, endpoint, json_body=json_body)

    # D-51: populate cache for GET responses; evict if over capacity
    if cacheable:
